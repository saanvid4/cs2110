...
/**
 * This object computes and remembers shortest paths through a weighted, directed graph. Once
 * shortest paths are computed from a specified source vertex, it allows querying the distance to
 * arbitrary vertices and the best paths to arbitrary destination vertices.
 * <p>
...
public class ShortestPaths<Vertex, Edge> {
    ...
    public void singleSourceDistances(Vertex source) {
        ...
        while (!frontier.isEmpty()) {
            ...
            for (Edge e : graph.outgoingEdges(g)) {
                Vertex v = graph.dest(e);
                double d = graph.weight(e);
                double weight = d + distances.get(g);
                if (!distances.containsKey(v)) {
                    frontier.add(v, weight);
                    distances.put(v, weight);
                    bestEdges.put(v, e);
                } else {
                    if (weight < distances.get(v)) {
                        distances.put(v, weight);
                        frontier.changePriority(v, weight);
                        bestEdges.put(v, e);
                    }
    ...
    /**
     * Returns: the distance from the source vertex to the given vertex. Checks: distances have been
     * computed from a source vertex, and vertex v is reachable from that vertex.
     */
    ...
    public List<Edge> bestPath(Vertex target) {
        ...
        while (true) {
            ...
            if (e == null) {
                break; // must be the source vertex
            }
            path.addFirst(e);
            v = graph.source(e);
        }
